...
4. 中文股票名必须优先使用 Tushare `stock_basic` 生成的 `config/symbol_cache.json` 全量映射；代码内置映射只作为缓存不存在时的离线兜底。
5. 如果中文名无法唯一匹配，停止执行并要求用户补充标准代码。
6. 未写时间范围时默认最近两年。
7. 未写分析周期时默认中线。
8. `/股票` 输入多个股票时逐只生成单股报告；横向比较使用 `/观察池`。

## 数据流程

```text
解析用户命令
→ 使用 TUSHARE_TOKEN 刷新 Tushare 股票名称全量映射缓存
→ 生成 request.json
...
- 涨跌停风险：每日涨停价、跌停价、距涨停/跌停百分比和对应 `risk_flags`。
